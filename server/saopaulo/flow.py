...
#                        [cell_identifier + ['ID_ORDEM_O', start_station_index, end_station_index, 'lat', 'lon']]
    ...
    with_od_cells = with_origin_cells.merge(grid_and_stations, left_on=end_station_index, right_on=station_index)[cell_start_ids + cell_identifier + [start_station_index, end_station_index, 'trip counts',
...
def weighted_counts(with_od_cells, cell_identifier):
    ...
    normalized['start_lat'] = normalized['start_lat'] * \
        normalized['normalized_counts']
    normalized['start_lon'] = normalized['start_lon'] * \
        normalized['normalized_counts']
    normalized['end_lat'] = normalized['end_lat'] * \
        normalized['normalized_counts']
    normalized['end_lon'] = normalized['end_lon'] * \
        normalized['normalized_counts']
    ...
    od_counts = od_counts[cell_identifier +
                          ['trip counts', 'origin', 'destination', 'ID_ORDEM']]
    return od_counts
...
def od_countings2(trips, grid, stations, cell_identifier=['i', 'j'],
                  cell_start_ids=['i_start', 'j_start'], cell_end_ids=['i_end', 'j_end'],
                  station_index='station id', start_station_index='start station id', end_station_index='end station id'):
    grid = grid.geodataframe()
    ...
    grid_trips['geometry'] = grid_trips.apply(lambda row: Point(
        row['dest_lon'], row['dest_lat']), axis=1)
    trips_geoframe = gpd.GeoDataFrame(grid_trips, crs='epsg:4326')
    trips_geoframe = trips_geoframe.drop('index_right', 1)
    grid_trips = gpd.sjoin(grid, trips_geoframe, op='contains')
    grid_trips.rename(columns={'i': 'i_end', 'j': 'j_end'}, inplace=True)
    grid_trips.rename(columns={'orig_lat': 'start_lat', 'orig_lon': 'start_lon',
                               'dest_lat': 'end_lat', 'dest_lon': 'end_lon'}, inplace=True)
    grid_trips['ID_ORDEM'] = grid_trips['ID_ORDEM'].astype(str)
    return weighted_counts(grid_trips, cell_start_ids + cell_end_ids)
...
def od_countings_simple(trips, grid, stations):
    ...
    trips['start geometry'] = trips.apply(lambda row: Point(
        row['start station longitude'], row['start station latitude']), axis=1)
    trips_geoframe = gpd.GeoDataFrame(trips, crs='epsg:4326')
    ...
    with_origin_cells.columns = ['i_start',
                                 'j_start', 'trip counts', 'end station id']
    ...
    with_dest_cells.columns = ['i_start',
                               'j_start', 'i_end', 'j_end', 'trip counts']
    ...
    od_counts = od_cell_counts.merge(grid.geodataframe(), left_on=['i_start', 'j_start'], right_on=['i', 'j']) \
                              .merge(grid.geodataframe(), left_on=['i_end', 'j_end'], right_on=['i', 'j'])[['i_start', 'j_start', 'geometry_x', 'i_end', 'j_end', 'geometry_y', 'trip counts']]
    od_counts.columns = ['i_start', 'j_start', 'origin',
                         'i_end', 'j_end', 'destination', 'trip counts']
    od_counts['origin'] = od_counts['origin'].apply(
        lambda cell: Point(cell.centroid.x, cell.centroid.y))
    od_counts['destination'] = od_counts['destination'].apply(
        lambda cell: Point(cell.centroid.x, cell.centroid.y))
...
def flow_map(fmap, od_df, grid, stations, minimum=-1, maximum=-1, show=4, radius=1.0, text=POPUP_NUM_TRIPS,
    ...
    i = 1
    the_filter = od_df[start_cell_identifier[0]
                       ] == od_df[end_cell_identifier[0]]
    while i < len(start_cell_identifier):
        the_filter = the_filter & (
            od_df[start_cell_identifier[i]] == od_df[end_cell_identifier[i]])
        i += 1
    ...
    filtered = filtered[((filtered['trip counts'] >= minimum)
                         & (filtered['trip counts'] <= maximum))]
...
def flow_map_zones(fmap, od_df, minimum=-1, maximum=-1, show=4, radius=1.0, text=POPUP_NUM_TRIPS,
    ...
    # eliminate round-trips to the same station, which are not considering in this analysis
    no_rounding_trips = od_df[od_df[start_zone_identifier]
                              != od_df[end_zone_identifier]]
    ...
    no_rounding_trips = no_rounding_trips[((no_rounding_trips['trip counts'] >= minimum) & (
        no_rounding_trips['trip counts'] <= maximum))]
