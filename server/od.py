...
class OD:
  ...
  def trips_by_start_time(self, trips, periods, specific, min_hour, max_hour):
    ...
    for period in periods:
      ...
      if period == 'afternoon':
        filtered_trips = filtered_trips.append(trips[(trips['HORA_SAIDA'].dt.hour >= 12) & (trips['HORA_SAIDA'].dt.hour <= 18)])
      if period == 'evening':
  ...
  def coords_by_tier(self, trips, base_layer, num_tiers=4):
    ...
    for index, row in tiers_table.iterrows():
      flows_by_tier[index] = self.zones.apply_od_flows(od, row['min'], row['top'], globalMaximum)
